...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body && body.login && body.password) {
      ...
      if (
        login != process.env.ADMIN_LOGIN_SECRET ||
      ...
      if (ghUsers) return NextResponse.json({ ghUsers: ghUsers }, { status: 200 });
      else return NextResponse.json({ ghUsers: [] }, { status: 200 });
    } else return NextResponse.json({ error: "Requisição invalida" }, { status: 400 });
  } catch (error) {
